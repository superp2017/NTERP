...
FormMaterManager::FormMaterManager(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::FormMaterManager)
{
    ui->setupUi(this);
...
void FormMaterManager::closeAllStatus()
{
...
void FormMaterManager::on_pushButton_mod_clicked()
{
...
void FormMaterManager::on_pushButton_del_clicked()
{
    for(int i=0;i<m_boxs.size();++i){
        QCheckBox *box = m_boxs.at(i);
        if(box!=NULL&&box->isChecked()){
...
void FormMaterManager::on_pushButton_export_clicked()
{
    QVector<Materiel> ls;
    ...
    if(!filepath.isEmpty()){
        boost::thread t(boost::bind(&FormMaterManager::doExport,this,ls,filepath));
        t.detach();
...
void FormMaterManager::on_pushButton_exit_clicked()
{
    this->hide();
}

void FormMaterManager::checkBox()
{
    bool check = true;
    int checkSize = 0;
...
void FormMaterManager::exportCb(bool ok)
{
...
void FormMaterManager::checkAll()
{
...
void FormMaterManager::cellDoubleClicked(int row, int col)
{
    ui->tableWidget->checkSelect();
...
void FormMaterManager::changeCol()
{
...
void FormMaterManager::delMaterCb(QString MID, bool ok)
{
    dataCenter::instance()->pub_hideLoadding();
    if(ok){
...
void FormMaterManager::setRowData(Materiel ma, int row)
{
    QCheckBox *box ;
    QWidget *w= ui->tableWidget->cellWidget(row,0);
    if(w ==NULL){
...
void FormMaterManager::removeOne(QString ma)
{
    int count = ui->tableWidget->rowCount();
    for(int i=0;i<count;++i){
        QWidget *w = ui->tableWidget->cellWidget(i,0);
...
void FormMaterManager::appendOne(Materiel ma)
{
...
void FormMaterManager::doExport(QVector<Materiel> ls, QString filepath)
{
